...
-- ── กติกา ────────────────────────────────────────────────────────────────────
Config.MaxPlantsPerZone  = 40  -- ต่อ "หนึ่งตัวละคร ต่อหนึ่งโซน" — server บังคับจริง
Config.PlantTimeoutHours = 1   -- ปลูกแล้วไม่ทำให้ครบใน 1 ชม. = ลบทิ้ง (นับจากตอนปลูก)
...
-- ระยะจากตัวผู้เล่นที่ต้นจะไปลง — 0 = ตรงจุดที่ยืนพอดี
Config.PlantDistance  = 0.0

Config.InteractRange  = 1.2  -- ระยะที่ prompt ลอยขึ้นเหนือต้น — ลดตาม minDistance ที่แคบลง
                             -- ไม่งั้นยืนจุดเดียวจะมีหลายต้นอยู่ในระยะพร้อมกัน
...
-- ── ฝนตก = รดน้ำให้เอง ───────────────────────────────────────────────────────
-- ต้นที่อยู่ stage 'water' (ใส่ปุ๋ยแล้ว รอรดน้ำ) จะถูกรดให้อัตโนมัติเมื่อฝนตก
-- ไม่เสียน้ำในถัง ไม่ต้องเดินไปกด — ข้ามไป stage 'grow' แล้วเริ่มนับเวลาโตทันที
-- (ต้นที่ยังไม่ใส่ปุ๋ยไม่ได้รับผล ต้องใส่ปุ๋ยก่อนเสมอ)
Config.RainWatering = {
    enabled      = true,
    checkSeconds = 30,   -- server เช็คสภาพอากาศทุกกี่วินาที
    -- ชื่อสภาพอากาศจาก weathersync (exports.weathersync:getWeather() ฝั่ง server)
    -- ตั้ง false / ลบออก ถ้าไม่อยากให้แบบไหนนับเป็นฝน (หิมะ/ลูกเห็บไม่นับไว้ก่อน)
    weathers = {
        ['rain']         = true,
        ['drizzle']      = true,
        ['shower']       = true,
...
Config.Zones = {
    valentine_farm = {
        ...
        minDistance = 1.0, -- ระยะห่างขั้นต่ำระหว่างต้น
