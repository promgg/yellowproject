...
-- tostring() ทั้งสองฝั่ง: Character.charIdentifier เป็น number (characters.charidentifier คือ int(11)
-- ใน DB) แต่ charIdentifier ที่รับมาอ่านจาก lp_marketplace.seller_id ซึ่งเป็น VARCHAR(64) →
-- oxmysql คืนมาเป็น string เทียบ "42" == 42 ตรงๆ ด้วย Lua == จะเป็น false เสมอ (คนละชนิดข้อมูล)
function FindSourceByCharIdentifier(charIdentifier)
    for _, playerId in ipairs(GetPlayers()) do
        local src = tonumber(playerId)
        local char = GetCharacter(src)
        if char and tostring(char.charIdentifier) == tostring(charIdentifier) then
            return src
        end
    end
    return nil
