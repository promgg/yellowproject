...
-- ── Daily Login ────────────────────────────────────────────────────────────
-- ⚠️ ผูกกับ "วันที่จริงในปฏิทิน" — การ์ดใบที่ N = วันที่ N ของเดือน ไม่ใช่ streak
--    เข้าวันที่ 1 แล้วหายไปโผล่วันที่ 30 = ได้แค่ใบ 1 กับ 30 ใบ 2-29 เสียถาวร
--    รอบรีเซ็ตตอนขึ้นเดือนใหม่ (ล้าง claimed ทั้งแถวฟรีและแถวทอง)
--
--    cycleDays คงไว้ที่ 30 โดยตั้งใจ ไม่ยืดหดตามความยาวเดือน:
--      เดือน 31 วัน -> วันที่ 31 ไม่มีการ์ด ไม่มีรางวัล
--      กุมภาพันธ์   -> การ์ดใบ 29-30 (และ 30 ในปีอธิกสุรทิน) ไม่มีวันถึงคิว เคลมไม่ได้ทั้งเดือน
--
--    VIP รับย้อนหลังได้ทั้งเดือน ไม่จำกัดจำนวนวัน และครอบทั้งสองแถว
--    = ซื้อ vip_card วันสุดท้ายของเดือนแล้วกวาดรวดเดียวได้ตามที่ตั้งใจ (เป็นจุดขายของ VIP)
...
Config.Locale = {
    ...
    claimLocked   = 'ยังไม่ถึงวันนี้',
    claimMissed   = 'วันนี้ขาดไปแล้ว รับย้อนหลังไม่ได้ (VIP รับย้อนหลังได้)',
