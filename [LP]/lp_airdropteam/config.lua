...
Config.Team = {
    ...
    --
    -- ปิดไว้ตั้งแต่ย้ายทางเข้าไปอยู่ในเมนู Fast Travel ของ lp_fasttravel แล้ว
    -- (joinCoords ทั้ง 3 จุดอยู่ห่างจากสถานี fast travel ไม่กี่เมตร NPC สองตัวเลยยืนทับกัน)
    -- เปิดกลับเป็น true ได้ถ้าอยากใช้ NPC แทน/ควบคู่กับปุ่มในเมนู — ทำงานร่วมกันได้ ไม่ชนกัน
    npc = {
        enabled  = false,
    ...
    -- ใช้ marker ตัวเดียวกับ "โดมหลัก" ของโซนแอร์ดอป (core/client.lua) ต่างแค่สี/รัศมี
    -- marker ของ RedM ใช้ hash ไม่ใช่เลข type แบบ GTA — อย่าเปลี่ยน hash เป็นตัวเลขเฉยๆ
    -- ไม่งั้นโดมจะไม่ขึ้น (เคยพลาดมาแล้วตอนใช้ DrawMarker แบบ type 1 / 28)
    safeZoneMarker = {
        enabled = true,
        hash    = 0x94FDAE17,   -- โดมเดียวกับโซนแอร์ดอป
        r = 255, g = 40, b = 40, a = 100,
        zOffset = -10.0,        -- ฐานโดมต่ำกว่าพื้น เหมือนที่โดมหลักทำ
    },
