...
-- ── ล็อกปุ่มเก็บ/ปล่อยปลา ตอนเพิ่งดึงปลาขึ้นมา (state 12) ────────────────────
-- บั๊กที่แก้: กดคลิกซ้ายพร้อมจังหวะที่กด space จบมินิเกมพอดี จะข้ามอนิเมชันดึงปลา
-- ขึ้นมาได้เลย (ตัว handler ของคลิกซ้าย/ขวาใน state 12 ไม่มีตัวกั้นเวลา ยิงได้ตั้งแต่
-- เฟรมแรกที่เข้า state) — ยิ่งตอนมินิเกมเป็น NUI ที่ถือ focus อยู่ พอ focus หลุด
-- คลิกที่ค้างอยู่จะถูกเกมนับเป็น "เพิ่งกด" ในเฟรมแรกของ state 12 พอดี
--
-- ตั้งเป็น 0 = ปิดการล็อก (กดข้ามได้ทันทีเหมือนเดิม)
-- ค่านี้ทำงานแยกจาก LandingBar.Enabled — ปิดหลอดแต่ยังล็อกปุ่มได้
Config.LandingInputLockMs = 1500
...
Config.FishingGuard = {
    ...
    -- state ที่ "เอ็นออกไปแล้ว" — สกรอลล์ตอนนี้จะตัดการตกกลางคัน (3 = ท่าง้าง)
    --
    -- ⚠️ ห้ามใส่ 1 กับ 13 กลับเข้ามา:
    --   1  = ถือเบ็ดพร้อมตก (ยังไม่เหวี่ยง)   13 = เพิ่งหยิบเบ็ดขึ้นมา
    -- สองอันนี้คือสถานะ "ถือเบ็ดอยู่เฉยๆ" ซึ่งเป็น state ที่ค้างอยู่ตลอดเวลาที่ยังถือเบ็ด
    -- (รวมถึงหลังตกปลาเสร็จแล้วด้วย) ถ้าบล็อกด้วยจะเปิด weapon wheel / สลับอาวุธไม่ได้เลย
    -- ตราบใดที่เบ็ดยังอยู่ในมือ — ออกได้ทางเดียวคือกดเก็บอาวุธ
    --
    -- ไม่บล็อกสองอันนี้แล้วเหยื่อก็ไม่หาย เพราะมีระบบ auto-resume อยู่แล้ว: สลับอาวุธไป
    -- แล้วกลับมาถือเบ็ดใหม่ จะตกต่อจากเหยื่อเดิมที่ค้างอยู่ (currentLure ไม่ถูกล้างตอนเก็บเบ็ด)
    BlockDuringStates = { 2, 3, 6, 7, 12 },
}
...
--
-- radius = 359.74 ม. คำนวณจากระยะ ศูนย์กลางไร่ Rhodes (967.7547, -1938.8070)
-- ถึงจุดขอบเขตห้ามตกปลาที่วัดจริง (859.0538, -1595.8829) แล้วนำค่านี้ไปใช้กับ
-- ทั้ง 3 เมืองปลูกผัก (ศูนย์กลางไร่ดึงจาก lp_planting/config.lua Config.Zones)
Config.NoFishZones = {
    { label = 'ไร่ Valentine', coords = vector3(-847.4569,  320.4838,  95.5757), radius = 359.74 },
    { label = 'ไร่ Annesburg', coords = vector3(2967.7837,  773.5686,  51.3994), radius = 359.74 },
    { label = 'ไร่ Rhodes',    coords = vector3(967.7547,  -1938.8070, 46.7184), radius = 359.74 },
}
