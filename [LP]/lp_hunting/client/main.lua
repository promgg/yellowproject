--[[
    ...
    flow (UI ของเราเอง + แอนิเมชันถลกจริงของเกม):
      ...
      3) ค้าง [E] ครบ -> เช็ค lp_hunting:cb:canSkin กับ server ก่อน (มีมีดไหม / กระเป๋าเต็มไหม) ถ้าไม่ผ่าน:
         ...
         — ผู้เล่นต้อง "ถือมีดอยู่ในมือ" เองก่อน (isHoldingKnife) ระบบไม่หยิบให้อัตโนมัติแล้ว
         ถ้าไม่ถือจะเด้ง pNotify แจ้งแล้วไม่เริ่มถลก จากนั้น:
...
-- ต้องถือมีดอยู่ในมือจริงตอนกดชำแหละ (ไม่ใช่แค่มีในกระเป๋า)
-- เช็คได้เฉพาะฝั่ง client — server มองไม่เห็นว่าอะไรอยู่ในมือ เลยยังเช็ค "เป็นเจ้าของมีด" ไว้เป็น
-- ด่านล่างสุดของมันเอง กติกา "ต้องถือ" เป็นกติกาการเล่น ไม่ใช่เส้นแบ่งความปลอดภัย
local KNIFE_HASHES = {}
for _, name in ipairs(Config.KnifeWeapons or {}) do
    KNIFE_HASHES[GetHashKey(name)] = true
end

local function isHoldingKnife()
    local ped = PlayerPedId()
    local ok, weapon = GetCurrentPedWeapon(ped, true, 0, true)
    if not ok or not weapon then return false end
    return KNIFE_HASHES[weapon] == true
end
                    ...
                    exports.lp_textui:TextUIHold(
                        ...
                        function()
                            if activeCarcass == target and DoesEntityExist(target) and IsPedDeadOrDying(target, true) then
                                -- ต้องถือมีดอยู่ในมือจริงตอนนี้ ไม่ใช่แค่มีในกระเป๋า และระบบจะไม่หยิบให้อัตโนมัติ
                                -- เช็คก่อนถามอย่างอื่น เพื่อให้ข้อความตรงเหตุที่สุด
                                if not isHoldingKnife() then
                                    exports.pNotify:SendNotification({
                                        text = Config.RequireKnifeMsg or 'คุณไม่มีมีดที่จะใช้แล่',
                                ...
                                if not result or not result.ok then
                                    ...
                                    if reason == 'noKnife' then
                                        msg = Config.RequireKnifeMsg or 'คุณไม่มีมีดที่จะใช้แล่'
                                    end
