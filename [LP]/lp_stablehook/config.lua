...
-- ── ปิดปุ่ม "ซื้อด้วยทอง" (BUY WITH GOLD) ────────────────────────────────────
-- ตั้ง false เพื่อคืนปุ่มซื้อด้วยทองกลับมา — เหลือแต่ซื้อด้วยเงินสด
...
-- สร้างรายการตาม flag เพื่อให้เปิด/ปิดแต่ละปุ่มได้อิสระ
Config.BlockPrompts = {}
if Config.DisableBequeath then
    Config.BlockPrompts[#Config.BlockPrompts + 1] = 'bequeath'
end
if Config.DisableGoldBuy then
    -- ⚠️ "buyGold" เดาจาก Config.keys.buyGold ของ kd_stable — ต้องยืนยันด้วย Dump ก่อน (ดูหมายเหตุท้ายไฟล์)
    Config.BlockPrompts[#Config.BlockPrompts + 1] = 'buyGold'
end
...
Config.Dump = {
    enabled       = true, -- เปิดชั่วคราวเพื่อหาชื่อ prompt ปุ่มทองในหน้าซื้อม้า (ปิดกลับเป็น false เมื่อยืนยันแล้ว)
