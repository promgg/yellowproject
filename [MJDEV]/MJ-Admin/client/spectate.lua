...
local Cam
local nextC, prevC, exitC
local SpectatePrompts = GetRandomIntInRange(0, 0xffffff)

Citizen.CreateThread(function()

    str = 'Exit Spectate'
    exitC = PromptRegisterBegin()
    PromptSetControlAction(exitC, 0xA65EBAB4)
    str = CreateVarString(10, 'LITERAL_STRING', str)
    PromptSetText(exitC, str)
    PromptSetEnabled(exitC, true)
    PromptSetVisible(exitC, true)
    PromptSetStandardMode(exitC, 1)
    PromptSetGroup(exitC, SpectatePrompts)
    PromptRegisterEnd(exitC)
end)
...
local function stopSpectating()
    ...
    TriggerServerEvent('MJDEV1993:req:spectate:end')
end
        ...
        while isSpectateEnabled and storedTargetServerId == initialTargetServerid do
            ...
            if not DoesEntityExist(storedTargetPed) then
                ...
                if newPed > 0 then
                    ...
                    debugPrint(("Spectated player (%s) no longer exists, ending spectate..."):format(
...
local function handleSpecCycle(isNext)
    ...
    debugPrint(('Cycling spectate from target: %s, isNext: %s'):format(tostring(storedTargetServerId), tostring(isNext)))
    TriggerServerEvent('MJDEV1993:req:spectate:cycle', storedTargetServerId, isNext)
end
...
local checkControlsFunc = redmCheckControls
...
local function createInstructionalThreads()
    ...
    -- drawing thread
    CreateThread(function()
        local fivemScaleform = IS_FIVEM and makeFivemInstructionalScaleform(keysTables)
        while isSpectateEnabled do
            local label = CreateVarString(10, 'LITERAL_STRING', 'Spectate')
            PromptSetActiveGroupThisFrame(SpectatePrompts, label)
            Wait(7)
        end
...
RegisterNetEvent('MJDEV1999:spectate:cycleFailed', function()
...
RegisterNetEvent('MJDEV1999:spectate:start', function(targetServerId, targetCoords)
    print(targetServerId, targetCoords)
    if isInTransitionState then
        stopSpectating()
    end
...
RegisterNetEvent("admin:showFlagWar")
AddEventHandler("admin:showFlagWar", function(Plays)
    if not isMenuVisible then
        ...
        MenuData.Open('default', GetCurrentResourceName(), 'allmennua', {
            title = "<p><span style='color:gold;'>ข้อมูลผู้เล่นในการแข่ง</span>",
            subtext = '!',
            align = 'top-right',
            elements = elements
        }, function(data, menu)
            if (data.current.value) then
                TriggerServerEvent('MJDEV1993:req:spectate:start', data.current.value)
...
RegisterCommand('spectate', function()
    VORPcore.RpcCall("admin:fetchUserRank", function(playerRank)
        _playerRank = playerRank
        print(_playerRank)
        if _playerRank == 'admin' then
            TriggerServerEvent('admin:getDataFlagWar')
        end
    end)
end, false)
