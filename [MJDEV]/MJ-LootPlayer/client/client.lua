        ...
        for _, v in pairs(GetNearbyPlayer()) do
            if not active_menu and v.source ~= 0 and v.enable and not Player(v.source).state.Stealing then
                ...
                if PromptHasHoldModeCompleted(StealPrompt[v.ped]) then
                    TriggerServerEvent('MJ-LootPlayer:OpenMenu', v)
                    Wait(500)
                end

            elseif active_menu and not v.enable and LocalPlayer.state.DataSteal and LocalPlayer.state.DataSteal.source == v.source then
                TriggerServerEvent('MJ-LootPlayer:Stealing', LocalPlayer.state.DataSteal.source, false)
                LocalPlayer.state:set('DataSteal', nil, true)

                ClearPedTasks(PlayerPedId())
                TriggerServerEvent('MJ-LootPlayer:CloseInventory')
                active_menu = false
                -- MenuData.CloseAll()
            end
        end
...
RegisterNetEvent('MJ-LootPlayer:OpenMenu')
AddEventHandler('MJ-LootPlayer:OpenMenu', function(CharacterMoney)
    -- MenuData.CloseAll()
    ...
    -- })
    -- ClearPedTasks(PlayerPedId())
    -- TriggerServerEvent('MJ-LootPlayer:ReloadInventory', LocalPlayer.state.DataSteal.source)
    -- TriggerServerEvent('MJ-LootPlayer:OpenInventory', LocalPlayer.state.DataSteal.source)
        ...
        if data.current.value == 'money' then                                      --translate here same as the config
            ...
            TriggerEvent('vorpinputs:advancedInput', json.encode(myInput), function(result)
                ...
                if number and number <= CharacterMoney then
                    TriggerServerEvent('MJ-LootPlayer:StealMoney', LocalPlayer.state.DataSteal.source, number)
