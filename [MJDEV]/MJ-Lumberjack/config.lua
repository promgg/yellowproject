...
-- รัศมีสแกนต้นไม้ = ระยะที่ marker ขึ้นด้วย (marker ขึ้น "ทุกต้น" ในระยะนี้ ไม่ใช่แค่ต้นใกล้สุด)
-- ตั้ง 60 ให้ครอบโซนรัศมี 50 ได้ทั้งโซนแม้ยืนอยู่ริมขอบ
-- ยิ่งกว้างยิ่งกินแรงตอนวน itemset แต่สแกนแค่ทุก 500ms ไม่ใช่ทุกเฟรม
Config.TreeScanRange = 60.0
...
-- ต้นเดิมตัดซ้ำได้เมื่อครบเวลานี้ (แยกกันรายบุคคล — คนอื่นตัดต้นเดียวกันได้ทันที)
-- เดิมเลข 900000 ฝังอยู่ในโค้ด 2 ที่ (server บังคับจริง + client ซ่อน marker) ถ้าแก้ไม่ครบทั้งคู่
-- marker จะหายไม่ตรงกับที่ตัดได้จริง — ย้ายมารวมที่นี่ที่เดียวให้เหมือน RockCooldown ของ Mining
Config.TreeCooldown = 60000   -- 1 นาที (ms)
