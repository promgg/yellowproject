                        ...
                        if UiPromptHasStandardModeCompleted(prompt, 0) then
                            if isOnDuty() then
                                ...
                                if not isAnyPlayerClose then
                                    TriggerServerEvent("MJ-Medic:Server:OpenStorage", key)
                                else
...
RegisterNetEvent("MJ-Medic:Client:JobUpdate", function()
    ...
    MenuData.Open("default", GetCurrentResourceName(), "OpenDoctorMenu", {
        ...
        elseif data.current.value == "fire" then
            ...
            if res and res > 0 then
                TriggerServerEvent("MJ-Medic:server:firePlayer", res)
            end
        end
...
function OpenHireMenu()
    ...
    MenuData.Open("default", GetCurrentResourceName(), "OpenHireFireMenu", {
        ...
        res = tonumber(res)
        if res and res > 0 then
            TriggerServerEvent("MJ-Medic:server:hirePlayer", res, data.current.value)
        end
    end, function(data, menu)
        menu.close()
    end)
...
local function OpenMedicMenu()
    ...
    MenuData.Open("default", GetCurrentResourceName(), "OpenMedicMenu", {
        ...
        elseif data.current.value == "duty" then
            local result = Core.Callback.TriggerAwait("MJ-Medic:server:checkDuty")
            if result then
...
RegisterNetEvent("MJ-Medic:Client:OpenMedicMenu", function()
    OpenMedicMenu()
end)

RegisterNetEvent("MJ-Medic:Client:HealAnim", function()
    playAnimation("script_mp@player@healing", "healing_male")
end)

RegisterNetEvent("MJ-Medic:Client:ReviveAnim", function()
    playAnimation("mech_revive@unapproved", "revive")
end)

RegisterNetEvent("MJ-Medic:Client:HealPlayer", function(health, stamina)
...
RegisterNetEvent("MJ-Medic:Client:AlertDoctor", function(targetCoords)
...
RegisterNetEvent("MJ-Medic:Client:RemoveBlip", function()
