...
local function openDoctorMenu(source)
    ...
    TriggerClientEvent('MJ-Medic:Client:OpenMedicMenu', source)
end
...
--* OPEN STORAGE
RegisterNetEvent("MJ-Medic:Server:OpenStorage", function(key)
    ...
    local prefix = "MJ-Medic_storage_" .. key
    if Config.ShareStorage then
        prefix = "MJ-Medic_storage"
    end
...
--* HIRE PLAYER
RegisterNetEvent("MJ-Medic:server:hirePlayer", function(id, job)
    local _source <const> = source
    local user <const> = Core.getUser(_source)
    if not user then return end
    ...
    TriggerClientEvent("MJ-Medic:Client:JobUpdate", target)
...
--* FIRE PLAYER
RegisterNetEvent("MJ-Medic:server:firePlayer", function(id)
    local _source <const> = source
    local user <const> = Core.getUser(_source)
    if not user then return end
...
Core.Callback.Register("MJ-Medic:server:checkDuty", function(source, CB, _)
    local user <const> = Core.getUser(source)
    if not user then return end
...
AddEventHandler("vorp:playerJobChange", function(source, new, _)
    ...
    TriggerClientEvent("MJ-Medic:Client:JobUpdate", source)
end)
    ...
    for key, value in pairs(Config.Items) do
        Inv:registerUsableItem(key, function(data)
            ...
            if value.revive then
                ...
                TriggerClientEvent("MJ-Medic:Client:ReviveAnim", _source)
                ...
                local closestPlayer <const> = getClosestPlayer(_source)
                if not closestPlayer then return Core.NotifyObjective(_source, T.Player.NoPlayerFoundToRevive, 5000) end
                TriggerClientEvent("MJ-Medic:Client:HealAnim", _source)
                TriggerClientEvent("MJ-Medic:Client:HealPlayer", tonumber(closestPlayer), value.health, value.stamina)
            end
...
RegisterCommand(Config.AlertDoctorCommand, function(source)
    ...
    TriggerClientEvent("MJ-Medic:Client:AlertDoctor", closestDoctor, sourceCoords)
...
RegisterCommand(Config.cancelalert, function(source)
    ...
    if doctor > 0 then
        ...
        if user then
            TriggerClientEvent("MJ-Medic:Client:RemoveBlip", doctor)
...
RegisterCommand(Config.finishalert, function(source)
    ...
    if isOnCall and doctor > 0 then
        TriggerClientEvent("MJ-Medic:Client:RemoveBlip", _source)
