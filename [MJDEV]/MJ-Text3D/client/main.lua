RegisterNetEvent("MJ-Text3D:event")
local players = {}
local doPlayers = {}
        ...
        for a, b in next, players do
            ...
            if NetworkIsPlayerActive(p) then
                ...
                if DoesEntityExist(ped) then
                    ...
                    local ons, x, y = GetHudScreenPositionFromWorldPosition(coords.x, coords.y, coords.z+0.5)
                    if not ons then
                        x = (x*100)
                        y = (y*70)
            ...
            if NetworkIsPlayerActive(p) then
                ...
                if DoesEntityExist(ped) then
                    ...
                    if not ons then
                        x = (x*100)
                        y = (y*100)
                        str = str.."<p style=\"left: "..x.."%;top: "..y.."%;-webkit-transform: translate(-50%, 0%);max-width: 100%;position: fixed;text-align: center;color: #fc0303;background-color: #000000AA;border-radius:3px;\"><b>⠀🗨️ "..b.message.."⠀</b></p>"
                    end
                end
            end
        end
...
AddEventHandler("MJ-Text3D:event", function(source, message, type1)
