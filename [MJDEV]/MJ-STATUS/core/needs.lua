...
-- MJ_stress() คืนความเครียดเป็น 0..100 (สเกลจริงของ PlayerStatus.Stress — log โหลดค่าจริง ~1.2,
-- คอมเมนต์อ้าง "เกิน 90/ถึง 100", ตาราง shake 50..100 ใน config ก็ใช้สเกลนี้)
-- ระบบจอแดง+สั่นจอ (core/client.lua) เทียบค่านี้กับ Config.StressEffectThreshold (90)
--
-- ⚠️ เดิม export นี้ "ไม่เคยถูกนิยาม" แต่ core/client.lua เรียก exports['MJ-STATUS']:MJ_stress()
-- → call nil ทำให้เธรดจอแดง/สั่นจอ error ตายทั้งเธรดตั้งแต่รอบแรก (effect ตอนเครียดไม่ทำงานเลย)
-- ห้ามคืน nil เด็ดขาด ผู้เรียกเอาไปเทียบค่าต่อทันที
exports('MJ_stress', function()
    return math.max(0, math.min(100, tonumber(PlayerStatus.Stress) or Config.MinStress or 0))
end)
