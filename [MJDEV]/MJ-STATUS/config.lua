...
-- ── สเตมิน่าตอนวิ่ง ─────────────────────────────────────────────────────────
-- ค่าเริ่มต้นของเกมลดช้ามาก วิ่งได้เป็นนาทีโดยไม่เหนื่อย ตัวนี้บังคับให้หมดตามเวลาที่ตั้ง
--
-- นับจาก "หลอดเต็ม" — เริ่มวิ่งตอนเหลือครึ่งหลอดก็หมดในครึ่งเวลา
-- คิดจากค่าที่เหลือตอนเริ่มวิ่งเสมอ ไม่ใช่ลบทีละนิดต่อรอบ เพราะการลบต่อรอบจะไปบวก
-- กับการลดตามธรรมชาติของเกมด้วย แล้วเวลาจริงจะสั้นกว่าที่ตั้งไว้แบบเดาไม่ได้
Config.Stamina = {
    enabled       = true,
    drainSeconds  = 8.0,   -- วิ่งจากหลอดเต็มกี่วินาทีถึงหมด
    sprintOnly    = false, -- true = ลดเฉพาะตอนวิ่งเต็มสปีด (IsPedSprinting)
                           -- false = จ๊อกกิ้ง (IsPedRunning) ก็ลดด้วย
    tickMs        = 100,   -- ความถี่ในการปรับ (ถี่ไปหลอดกระตุก ห่างไปลดเป็นขั้น)
}
