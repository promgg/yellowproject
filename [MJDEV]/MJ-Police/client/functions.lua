...
function HandcuffPlayer() --Handcuff player function
    ...
    if closestDistance <= 3.0 then
        if not isDead then
            TriggerServerEvent('MJ-Police:handcuff', targetplayerid)
...
function CuffPlayer(closestPlayer) -- Prompt and code to access Gun Cabinets
    while true do
        ...
        if PromptHasHoldModeCompleted(Search) then
            TriggerServerEvent('MJ-Police:grabdata', GetPlayerServerId(closestPlayer))
            Wait(200)
            if Takenmoney then
...
function PutInOutVehicle()
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        TriggerServerEvent('MJ-Police:GetPlayerWagonID', GetPlayerServerId(closestPlayer))
    else
        VORPcore.NotifyBottomRight(_U('notcloseenough'), 4000)
        return
    end
