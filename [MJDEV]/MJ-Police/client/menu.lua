    ...
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
        ...
        function(data, menu)
            if (data.current.value == 'star') then
                TriggerServerEvent('MJ-Police:checkjob')
            ...
            elseif (data.current.value == 'escort') then
                local closestPlayer, closestDistance = GetClosestPlayer()
                if closestPlayer ~= -1 and closestDistance <= 3.0 then
                    TriggerServerEvent('MJ-Police:drag', GetPlayerServerId(closestPlayer))
                else
                    VORPcore.NotifyBottomRight(_U('notcloseenough'), 4000)
                end
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
        ...
        function(data, menu)
            ...
            elseif (data.current.value == 'jail') then
                ...
                TriggerServerEvent('MJ-Police:JailPlayer', tonumber(Playerid), tonumber(timeinjail), JailID)
            ...
            elseif (data.current.value == 'unjail') then
                TriggerServerEvent('MJ-Police:unjailed', Playerid, JailID)
            end
        end,
        function(data, menu)
            menu.close()
...
function OpenFineMenu() -- Fine Menu logic
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
        ...
        function(data, menu)
            ...
            elseif (data.current.value == 'fine') then
                TriggerServerEvent("MJ-Police:FinePlayer", tonumber(Playerid), tonumber(fineamount))
            end
        end,
        function(data, menu)
            menu.close()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
        ...
        function(data, menu)
            ...
            elseif (data.current.value == 'service') then
                TriggerServerEvent("MJ-Police:CommunityService", tonumber(Playerid), chore, tonumber(Choreamount))
            end
        end,
        function(data, menu)
            menu.close()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
        ...
        function(data, menu)
            ...
            elseif data.current.value == "revolver1" then
                TriggerServerEvent("MJ-Police:guncabinet", ConfigCabinets.WeaponsandAmmo.RevolverSpawnName1)
                CloseMenu()
            elseif data.current.value == "knife" then
                TriggerServerEvent("MJ-Police:guncabinet", ConfigCabinets.WeaponsandAmmo.KnifeSpawnName)
                CloseMenu()
            elseif data.current.value == "lasso" then
                TriggerServerEvent("MJ-Police:guncabinet", ConfigCabinets.WeaponsandAmmo.LassoSpawnName)
                CloseMenu()
            elseif data.current.value == "revolver2" then
                TriggerServerEvent("MJ-Police:guncabinet", ConfigCabinets.WeaponsandAmmo.RevolverSpawnName2)
                CloseMenu()
            elseif data.current.value == "shotgun" then
                TriggerServerEvent("MJ-Police:guncabinet", ConfigCabinets.WeaponsandAmmo.ShotgunSpawnName)
                CloseMenu()
            elseif data.current.value == "rifle" then
                TriggerServerEvent("MJ-Police:guncabinet", ConfigCabinets.WeaponsandAmmo.RifleSpawnName)
                CloseMenu()
            elseif data.current.value == "repeater" then
                TriggerServerEvent("MJ-Police:guncabinet", ConfigCabinets.WeaponsandAmmo.RepeaterSpawnName)
                CloseMenu()
            end
        end,
        function(data, menu)
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
        ...
        function(data, menu)
            ...
            elseif data.current.value == "ammo1" then
                ...
                TriggerServerEvent("MJ-Police:addammo", ammotype)
                Inmenu = false
                menu.close()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
        ...
        function(data, menu)
            ...
            elseif data.current.value == "getid" then
                local closestPlayer, closestDistance = GetClosestPlayer()
                if closestPlayer ~= -1 and closestDistance <= 3.0 then
                    TriggerServerEvent('MJ-Police:GetID', GetPlayerServerId(closestPlayer))
                end
            elseif data.current.value == "getowner" then
                local closestPlayer, closestDistance = GetClosestPlayer()
                if closestPlayer ~= -1 and closestDistance <= 3.0 then
                    local mount = GetMount(PlayerPedId())
                    TriggerServerEvent('MJ-Police:getVehicleInfo', GetPlayerServerId(closestPlayer), GetEntityModel(mount))
                else
                    local mount = GetMount(PlayerPedId())
                    local id = GetPlayerServerId(GetPlayerIndex())
                    TriggerServerEvent('MJ-Police:getVehicleInfo', id, GetEntityModel(mount))
                end
            end
        end,
        function(data, menu)
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'menuapi',
        ...
        function(data, menu)
            ...
            elseif data.current.value == "Items" then
                TriggerEvent('MJ-Police:StartSearch')
            end
        end,
        function(data, menu)
            menu.close()
