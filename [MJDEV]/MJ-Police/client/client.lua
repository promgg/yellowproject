...
Choreamount = _U('none')
Takenmoney, Search, InWagon = nil, nil, nil
local Brokedistance = false
local dragStatus = {}
        ...
        if Jailed then
            if JailID == "sk" then
                ...
                local currentCheck = GetDistanceBetweenCoords(Jailedcoords.x, Jailedcoords.y, Jailedcoords.z,
                    ...
                    ConfigJail.Jails.sisika.entrance.z, true)
                if currentCheck > 420 then
                    TriggerEvent("MJ-Police:breakout")
                end
            else
                local Jailedcoords = GetEntityCoords(PlayerPedId())
    ...
    while true do
        ...
        if Serviced then
                ...
                if IsControlJustReleased(0, 0xCEFD9220) then
                    if ConfigService.CommunityServiceSettings.minigame then
                        ...
                        if test == 100 then
                            ...
                            TriggerServerEvent("MJ-Police:updateservice")
                        else
            ...
            for k, v in pairs(ConfigService.construction) do
                local coords = GetEntityCoords(PlayerPedId())
                if GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) > ConfigService.CommunityServiceSettings.communityservicedistance then
                    Brokedistance = true
                else
                    Brokedistance = false
                end
        ...
        if Choreamount and Choreamount == 0 then
            TriggerServerEvent("MJ-Police:endservice")
            Serviced = false
...
RegisterNetEvent("MJ-Police:ServicePlayer") -- Assigns Chore amount and picks random coord for construcion
AddEventHandler("MJ-Police:ServicePlayer", function(chore, amount)
    Serviced = true
    Choreamount = amount
            ...
            if GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 1.5 and not Inmenu then
                if not isDead then
                    ...
                    if PromptHasHoldModeCompleted(Open) then
                        TriggerServerEvent("MJ-Police:PlayerJob") -- run client side check before check for distance. no need to run code that is not meant for the client its optimized this way
                        Wait(200)
...
RegisterNetEvent('MJ-Police:PlayerInWagon') -- Put in Vehicle logic, not in use currently
AddEventHandler('MJ-Police:PlayerInWagon', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('MJ-Police:StartSearch', function()
    local closestPlayer, closestDistance = GetClosestPlayer()
    searchid = GetPlayerServerId(closestPlayer)
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        TriggerServerEvent("MJ-Police:ReloadInventory", searchid)
        TriggerEvent("vorp_inventory:OpenstealInventory", _U('inventorytitle'), searchid)
    end
end)

RegisterNetEvent('MJ-Police:GetSearch')
AddEventHandler('MJ-Police:GetSearch', function(obj)
    TriggerServerEvent('MJ-Police:TakeFrom', obj, searchid)
end)

RegisterNetEvent("MJ-Police:PlayerJob") -- Job check event
AddEventHandler("MJ-Police:PlayerJob", function(Job)
    playerJob = Job
end)

RegisterNetEvent("MJ-Police:senddata") -- Job check event
AddEventHandler("MJ-Police:senddata", function(playermoney)
    Takenmoney = playermoney
end)

RegisterNetEvent("MJ-Police:breakout") -- Event for breaking out
AddEventHandler("MJ-Police:breakout", function()
    local coords = GetEntityCoords(PlayerPedId())
    local local_player = PlayerId()
    TriggerServerEvent('MJ-Police:jailbreak')
    TriggerServerEvent('MJ-Police:policenotify', coords)
...
AddEventHandler("vorp:SelectedCharacter", function(charid)
    TriggerServerEvent("MJ-Police:check_jail")
    Wait(200)
    TriggerServerEvent("MJ-Police:PlayerJob")
end)
...
-- Badges
RegisterNetEvent("MJ-Police:badgeon")
AddEventHandler("MJ-Police:badgeon", function(playerjob, jobgrade)
    Wait(60)
    local ped = PlayerPedId()
    ...
    if not badgeactive then
        ...
        if playerjob == "police" and jobgrade ~= 6 then
            if IsPedMale(ped) then
                ...
                AttachEntityToEntity(Badge, ped, MaleboneIndex, Badgex, Badgey, Badgez, -15.0, 0.0, Rotationz, true,
                    true, false, true, 1, true)
                BadgeCoords = GetEntityCoords(Badge)
            else
                Badge = CreateObject(GetHashKey("s_badgedeputy01x"), Badgex, Badgey, Badgez + 0.2, true, false, false)
                AttachEntityToEntity(Badge, ped, FemaleboneIndex, Badgex, Badgey, Badgez, -15.0, 0.0, 30.0, false, true,
                    false, true, 1, true)
                BadgeCoords = GetEntityCoords(Badge)
            end
        ...
        elseif playerjob == "marshal" and jobgrade ~= nil then
            if IsPedMale(ped) then
                Badge =
                    CreateObject(GetHashKey("s_badgeusmarshal01x"), Badgex, Badgey, Badgez + 0.2, true, false, false)
                AttachEntityToEntity(Badge, ped, MaleboneIndex, 0.17, -0.19, -0.25, -12.5, 0.0, 30.0, false, true,
                    false, true, 1, true)
                BadgeCoords = GetEntityCoords(Badge)
            else
...
RegisterCommand(ConfigMain.adjustbadgecommand, function()
    ...
    if CheckTable(OnDutyJobs, playerJob) and badgeactive then
        if not display then
            display = true
            ...
            while true do
                ...
                if display and badgeactive then
                    if IsPedMale(ped) then
                        ...
                        AttachEntityToEntity(Badge, ped, FemaleboneIndex, Badgex, Badgey, Badgez, -15.0, 0.0, Rotationz,
                            true, true, false, true, 1, true)
                    end
...
RegisterCommand(ConfigMain.openpolicemenu, function()
    if CheckTable(OnDutyJobs, playerJob) and not IsEntityDead(PlayerPedId()) then
        OpenPoliceMenu()
    else
        return
    end
...
RegisterNetEvent('MJ-Police:drag') -- Event to register dragging
AddEventHandler('MJ-Police:drag', function(copId)
    if IsHandcuffed then
...
RegisterNetEvent("MJ-Police:JailPlayer") -- Jailing player event
AddEventHandler("MJ-Police:JailPlayer", function(time, Location)
    local ped = PlayerPedId()
    ...
    if not Jailed then
        if Autotele then
            ...
            TriggerEvent("MJ-Police:wear_prison", ped)
            Wait(500)
        else
            Jail_time = time
            Jailed = true
...
RegisterNetEvent("MJ-Police:wear_prison") -- Wear prison outfit event
AddEventHandler("MJ-Police:wear_prison", function()
    local ped = PlayerPedId()
...
RegisterNetEvent("MJ-Police:UnjailPlayer") -- Unjail player event
AddEventHandler("MJ-Police:UnjailPlayer", function(jaillocation)
    ...
    if Autotele then
        if JailID == "sk" then
            SetEntityCoords(local_ped, ConfigJail.Jails.sisika.exit.x, ConfigJail.Jails.sisika.exit.y, ConfigJail.Jails.sisika.exit.z)
        elseif JailID == "bw" then
        ...
        elseif JailID == "rh" then
            SetEntityCoords(local_ped, ConfigJail.Jails.rhodes.exit.x, ConfigJail.Jails.rhodes.exit.y,
                ConfigJail.Jails.rhodes.exit.z)
        elseif JailID == "sd" then
...
CreateThread(function() -- Display timer when in jail logic
    while true do
        Wait(5)
        if Jailed then
            DrawTxt(_U('imprisoned') .. Jail_time .. _U('jailseconds'), 0.38, 0.95, 0.4, 0.4, true, 255, 0, 0, 255,
                false)
        else
            Wait(200)
        end
...
RegisterNetEvent("MJ-Police:cuffs") -- Cuffing player event
AddEventHandler("MJ-Police:cuffs", function()
    HandcuffPlayer()
end)

RegisterNetEvent("MJ-Police:lockpick") -- Lockpicking handcuffs event
AddEventHandler("MJ-Police:lockpick", function()
    local closestPlayer, closestDistance = GetClosestPlayer()
    local isDead = IsEntityDead(PlayerPedId())
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        ...
        if not isDead then
            if chance < 85 then
                ...
                Wait(325)
                repeat
                    TriggerEvent("MJ-Police:lockpick")
                until (chance)
            end
            if chance >= 85 then
                ...
                if breakChance < 3 then
                    TriggerServerEvent("MJ-Police:lockpick:break")
                else
                    ...
                    Citizen.Wait(1250)
                    TriggerServerEvent('MJ-Police:lockpicksv', GetPlayerServerId(closestPlayer))
                end
            end
        end
    else
    ...
    while true do
        if Jailed then
            ...
            if Jail_time < 1 then
                ...
                RemoveBlip(Serviceblip)
                Serviceblip = nil
                TriggerServerEvent("MJ-Police:finishedjail", player_server_id, jaillocation)
            else
...
CreateThread(function() -- Added time if over max distance/count down until unJailed logic
    while true do
        ...
        if Jailed then
            ...
            TriggerServerEvent("MJ-Police:taketime")
        else
            Wait(500)
        end
    end
...
RegisterNetEvent('MJ-Police:handcuff', function()
    local playerPed = PlayerPedId()
            ...
            if CheckTable(OnDutyJobs, playerJob) then
                if IsControlPressed(0, 0x8AAA0AD4) then
                    if IsDisabledControlPressed(0, 0xE30CD707) then
                        TriggerServerEvent('MJ-Police:CheckHandcuffs')
                    end
                end
            end
        end
...
RegisterNetEvent('MJ-Police:HandcuffsChecked')
AddEventHandler('MJ-Police:HandcuffsChecked', function(hasHandcuffs)
...
RegisterNetEvent('MJ-Police:lockpicked') -- Successful lockpick event
AddEventHandler('MJ-Police:lockpicked', function()
    local playerPed = PlayerPedId()
    ClearPedSecondaryTask(playerPed)
    SetEnableHandcuffs(playerPed, false)
    DisablePlayerFiring(playerPed, false)
...
CreateThread(function() -- Timer for leaving community service logic, which jails player
    while true do
        ...
        local seconds = ConfigService.CommunityServiceSettings.communityservicetimer -- max time (seconds) you want to set
        ...
        while Brokedistance do
            ...
            if printtime > 0 then
                ...
                DrawTxt(_U('youhave') .. printtime .. _U('secondsremaining'), 0.50, 0.95, 0.6, 0.6, true, 255, 255, 255, 255, true, 10000)
            else
                Citizen.Wait(1000)
                Brokedistance = false
                Serviced = false
                ...
                Location = "sk"
                local player_server_id = GetPlayerServerId(PlayerId())
                TriggerServerEvent('MJ-Police:JailPlayer', tonumber(player_server_id), tonumber(ConfigService.CommunityServiceSettings.leftserviceamount), JailID)
                TriggerServerEvent('MJ-Police:Jailedservice', source)
            end
        end
    end
end)

RegisterNetEvent("MJ-Police:witness", function(coords)
...
AddEventHandler('onResourceStart', function(resource) -- on resource restart remove Serviceblips
    if resource == GetCurrentResourceName() then
        TriggerServerEvent("MJ-Police:PlayerJob")
    end
end)
        ...
        while true do
            ...
            if dist < 5 then
                ...
                if IsControlJustReleased(0, 0x760A9C6F) then
                    TriggerServerEvent('MJ-Police:CommisaryAddItem')
                end
