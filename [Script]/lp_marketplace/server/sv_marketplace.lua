...
AddEventHandler('lp_marketplace:listItem', function(data)
    ...
    if data.itemName == '' or data.quantity <= 0 or data.price <= 0 or data.currency == '' then
        VORPcore.NotifyRightTip(src, Config.Locale.listed_fail, 4000)
        return
    end
    ...
    if not ok then
        VORPcore.NotifyRightTip(src,
            reason == 'rate_limited' and Config.Locale.rate_limited or Config.Locale.listed_fail, 4000)
        return
    end
    ...
    if restrict then
        ...
        if not allowed then
            VORPcore.NotifyRightTip(src, Config.Locale.restricted_item, 4000)
            return
        end
    end
    ...
    if (count or 0) >= Config.MaxListingsPerPlayer then
        VORPcore.NotifyRightTip(src,
            string.format(Config.Locale.max_listings, Config.MaxListingsPerPlayer), 4000)
        return
    end
    ...
    if ok2 and insertId then
        VORPcore.NotifyRightTip(src, Config.Locale.listed_success, 4000)
...
local function DoBuy(src, listingId, buyQty)
    ...
    if not ok then
        ...
        VORPcore.NotifyRightTip(src, Config.Locale.buy_fail, 4000)
        return false
    end
    return result
end
...
DoBuyInner = function(src, listingId, buyQty)
    ...
    if listing.seller_id == Character.charIdentifier then
        VORPcore.NotifyRightTip(src, Config.Locale.cannot_buy_own, 4000)
        return
    end
    ...
    if balance < totalPrice then
        VORPcore.NotifyRightTip(src, Config.Locale.not_enough_money, 4000)
        return
    end
    ...
    VORPcore.NotifyRightTip(src, Config.Locale.buy_success, 4000)
...
AddEventHandler('lp_marketplace:cancelListing', function(listingId)
    ...
    if not listing then
        VORPcore.NotifyRightTip(src, Config.Locale.cancel_fail, 4000)
        return
    end
    ...
    VORPcore.NotifyRightTip(src, Config.Locale.cancel_success, 4000)
    TriggerClientEvent('lp_marketplace:refreshSell', src)
...
AddEventHandler('lp_marketplace:claimItem', function(listingId)
    ...
    if not listing then
        VORPcore.NotifyRightTip(src, Config.Locale.claim_fail, 4000)
        return
    end
    ...
    VORPcore.NotifyRightTip(src, Config.Locale.claim_success, 4000)
