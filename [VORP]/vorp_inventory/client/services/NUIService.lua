...
function NUIService.TransferLimitExceeded(maxValue)
    local max = type(maxValue) == "table" and maxValue.max or 0
    local message = string.format(T.MaxItemTransfer, max)
...
function NUIService.NUIGiveItem(obj)
	...
	if type(obj) ~= "table" or type(obj.data) ~= "table" then return end
...
function NUIService.ContextMenu(data)
    ...
    if type(data.event) ~= "table" then return end

    if type(data.event.client) == "string" then
        TriggerEvent(data.event.client, data.arguments)
    elseif type(data.event.server) == "string" then
