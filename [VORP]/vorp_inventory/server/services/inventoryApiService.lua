...
function InventoryAPI.addItem(source, name, amount, metadata, cb, allow, degradation, percentage)
	...
	DBService.CreateItem(charIdentifier, svItem:getId(), amount, metadata or {}, name, isExpired, function(craftedItem)
		...
		exports['lp_itemnotify']:notification(_source, {
			position = 'middleRight', -- เลือกตำแหน่ง topLeft, topCenter, topRight, middleLeft, middleCenter, middleRight, bottomLeft, bottomCenter, bottomRight
			image = item:getName(), -- ใส่ชื่อไอเทมหรือชื้อไพล์ภาพใน image ต้องมีนามสกุนไพล์ตามหลัง
			title ='คุณได้รับ Item', -- ข้อความแรก
			description ='+ '.. amount, -- ข้อความสอง
			type ='success',-- type มี 4 type   success, alert, warning, info
			time = 4000, -- ใส่เวลา
...
function InventoryAPI.subItemID(player, id, cb, allow, amount)
	...
	if item:getCount() == 0 then
		...
		exports['lp_itemnotify']:notification(_source, {
			position = 'middleRight', -- เลือกตำแหน่ง topLeft, topCenter, topRight, middleLeft, middleCenter, middleRight, bottomLeft, bottomCenter, bottomRight
			image = itemName, -- ใส่ชื่อไอเทมหรือชื้อไพล์ภาพใน image ต้องมีนามสกุนไพล์ตามหลัง
			title ='คุณสูญเสีย Item', -- ข้อความแรก
			description ='- '.. amount, -- ข้อความสอง
...
function InventoryAPI.subItem(source, name, amount, metadata, cb, allow, percentage)
	...
	if exactMatchItem then
		...
		for _, value in ipairs(itemsToRemove) do
			...
			exports['lp_itemnotify']:notification(_source, {
				position = 'middleRight', -- เลือกตำแหน่ง topLeft, topCenter, topRight, middleLeft, middleCenter, middleRight, bottomLeft, bottomCenter, bottomRight
				image = name, -- ใส่ชื่อไอเทมหรือชื้อไพล์ภาพใน image ต้องมีนามสกุนไพล์ตามหลัง
				title ='คุณสูญเสีย Item', -- ข้อความแรก
				description ='- '.. amount, -- ข้อความสอง
				type ='warning',-- type มี 4 type   success, alert, warning, info
				time = 4000, -- ใส่เวลา
		...
		if itemFound then
			...
			if item:getId() ~= itemFound:getId() then
				...
				if item:getCount() == 0 then
					...
					exports['lp_itemnotify']:notification(_source, {
						position = 'middleRight', -- เลือกตำแหน่ง topLeft, topCenter, topRight, middleLeft, middleCenter, middleRight, bottomLeft, bottomCenter, bottomRight
						image = item.name, -- ใส่ชื่อไอเทมหรือชื้อไพล์ภาพใน image ต้องมีนามสกุนไพล์ตามหลัง
						title ='คุณสูญเสีย Item', -- ข้อความแรก
						description ='- '.. amount, -- ข้อความสอง
						type ='warning',-- type มี 4 type   success, alert, warning, info
						time = 4000, -- ใส่เวลา
