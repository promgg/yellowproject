...
-- ตั้งค่าจริงผ่าน convar ใน server.cfg: set poodlechat_discord_webhook_id "..."
-- หมายเหตุ: ต้นฉบับตั้งค่าฟิลด์นี้เป็น URL เต็มของ webhook ทั้งอัน (ไม่ใช่แค่ id ตามชื่อฟิลด์) —
-- คงพฤติกรรมเดิมไว้ตามที่เจอ ไม่ได้แก้ logic ส่วนนี้ เพราะไม่ได้ถูกขอให้ตรวจสอบ poodlechat
ServerConfig.DiscordWebhookId = GetConvar("poodlechat_discord_webhook_id", "")

-- Token for the above Discord webhook. Leave as '' to disable sending messages to Discord.
-- ตั้งค่าจริงผ่าน convar ใน server.cfg: set poodlechat_discord_webhook_token "..."
ServerConfig.DiscordWebhookToken = GetConvar("poodlechat_discord_webhook_token", "")

-- The default name to use on Discord.
ServerConfig.DiscordName = 'MJ DEV'
...
ServerConfig.DiscordChannel = '1347337537814790154'

-- Colour for messages from Discord
ServerConfig.DiscordColor = {115, 138, 219}

-- Discord webhook to post player reports to.
-- ตั้งค่าจริงผ่าน convar ใน server.cfg: set poodlechat_discord_report_webhook "https://discord.com/api/webhooks/..."
ServerConfig.DiscordReportWebhook = GetConvar("poodlechat_discord_report_webhook", "")
...
-- ตั้งค่าจริงผ่าน convar ใน server.cfg: set poodlechat_steam_key "..."
ServerConfig.SteamKey = GetConvar("poodlechat_steam_key", "")
